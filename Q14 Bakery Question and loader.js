...
// Prevent advancing past this page until exactly 30 items are selected.
// addOnNextButtonClick fires when the participant clicks Next; returning false
// blocks the page advance and keeps focus on the survey.
Qualtrics.SurveyEngine.addOnNextButtonClick(function() {
    var total = 0;
    jQuery(".shopping-category-question").each(function() {
        total += jQuery(this).find("input[type='checkbox']:checked").length;
    });
    if (total < 30) {
        jQuery("#cart-requirement")
            .show()
            .css({ "border-color": "#c0392b", "background": "#fff0f0", "color": "#c0392b" })
            .text("You must select exactly 30 items before continuing. (" + total + " selected so far)");
        document.getElementById("cart-requirement").scrollIntoView({ behavior: "smooth", block: "center" });
        return false;
    }
});
